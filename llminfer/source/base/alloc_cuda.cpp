...
void* CUDADeviceAllocator::allocate(size_t byte_size) const {
  /*old version
    always use cudaMalloc to malloc cuda memory
    for trivial memory, calling cudaMalloc api all the time is inefficient
  */
  // if (!byte_size) {
  //   return nullptr;
  // }
  ...
  /*new version
  use memory buffer to manage
  */
  int device_id = -1;
  cudaError state = cudaGetDevice(&device_id);
  if (byte_size > 1024 * 1024) { // > 1mb
    ...
    for (int i = 0; i < big_buffers.size(); i++) {
      if (big_buffers[i].byte_size >= byte_size && !big_buffers[i].busy &&
          big_buffers[i].byte_size < 1024 * 1024) {
        sel_id = i;
      }
    }
  ...
  for (int i = 0; i < cuda_buffers.size(); i++) {
    if (cuda_buffers[i].byte_size >= byte_size && !cuda_buffers[i].busy) {
      ...
      no_busy_cnt_[device_id] -= cuda_buffers[i].byte_size;
