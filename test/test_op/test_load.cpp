...
TEST(test_load, load_model_weight) {
  ...
  float* weight_data = reinterpret_cast<float*>(
      static_cast<int8_t*>(data) + sizeof(model::ModelConfig));
...
TEST(test_load, create_matmul) {
  ...
  tensor::Tensor tensor(
      base::DataType::kDataTypeFp32, config.hidden_dim, false, nullptr, in);
  tensor.set_device_type(base::DeviceType::kDeviceCPU);

  tensor::Tensor out_tensor(
      base::DataType::kDataTypeFp32, config.dim, false, nullptr, out);
